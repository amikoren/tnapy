...
static int fatal_error(char *err)
{
   exit(-1);
}
...
static void *realloc_e(void *ptr, size_t size)
{
    ...
    if (!p)
	fatal_error("Failed realloc memory");
    return p;
}
...
static void *malloc_e(size_t size)
{
    void *p = malloc(size);
    if (!p)
	fatal_error("Failed malloc memory");
    return p;
}
...
char **str_vprintf_full(char **s, int flags, char *fmt, va_list ap)
{
    ...
    if (rc<0)
	fatal_error("Error in printf format (%s)");
    if (!cat_len)
    {
	old_s = *s;
	*s = NULL;
    ...
    *s = realloc_e(*s, cat_len+rc+1);
    ...
    nfree(old_s);
    if (rc2 < 0)
	fatal_error("Failed vnsprintf"); /* this should never happen */
    return s;
}
